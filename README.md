...
Beyond those four flagship examples, this server monetizes **17 additional endpoints**: ten data endpoints covering the categories driving the most real volume across the x402 ecosystem today — prediction markets (Polymarket, Kalshi), DeFi (DefiLlama), crypto market data (CoinGecko), and web search (DuckDuckGo) — priced $0.001–$0.003, plus seven small free-public-API lookups priced $0.002. All are built on a shared `createX402Route()` helper (`app/api/_lib/x402Route.ts`) so every one of them gets the same `exact`-scheme EVM+Solana payment gating, Bazaar discovery metadata, and signed offer/receipt behavior with only a few lines of route-specific code. See "All monetized endpoints" below for the full list.
...
**Free-public-API lookups** (no key required upstream):

| Route | Price | Description | Upstream |
|---|---|---|---|
| `/api/dictionary` | $0.002 | English word definitions and phonetics | dictionaryapi.dev |
| `/api/currency` | $0.002 | Currency conversion, official ECB reference rates | Frankfurter API |
| `/api/dns` | $0.002 | DNS record lookup (A, AAAA, MX, TXT, NS, CNAME, SOA, CAA) | Cloudflare DNS-over-HTTPS |
| `/api/whois` | $0.002 | Domain registration status, dates, nameservers | RDAP (rdap.org) |
| `/api/github` | $0.002 | Public GitHub repo metadata (stars, forks, license, activity) | GitHub REST API |
| `/api/npm` | $0.002 | npm package metadata (latest version, license, publish history) | npm registry |
| `/api/wikipedia` | $0.002 | Wikipedia article summary by title | Wikipedia REST API |

22 monetized endpoints total. Every route's exact input schema and a live example call/response is discoverable via its `extensions.bazaar` metadata in the 402 response — see "Discovery & trust" below.
...
app/
  ...
  api/{dictionary,currency,dns,whois,github,npm,wikipedia}/route.ts  # 7 free-public-API lookup endpoints (see "All monetized endpoints" for upstreams)
