...
Beyond those four flagship examples, this server monetizes **15 additional endpoints**: eleven data endpoints covering the categories driving the most real volume across the x402 ecosystem today — prediction markets (Polymarket, Kalshi, plus a cross-platform divergence/arbitrage finder), DeFi (DefiLlama), crypto market data (CoinGecko), and real web search (Exa) — plus four on-chain-data endpoints (multi-chain RPC, wallet balance, gas price, ENS resolution) mirroring the same "on-chain data" category BlockRun (the largest x402 seller by settlement volume) sells as a generic RPC proxy. All priced $0.001–$0.005. All are built on a shared `createX402Route()` helper (`app/api/_lib/x402Route.ts`) so every one of them gets the same `exact`-scheme EVM+Solana payment gating, Bazaar discovery metadata, and signed offer/receipt behavior with only a few lines of route-specific code. See "All monetized endpoints" below for the full list.
...
**Also ships as an MCP server.** [`microtap-mcp`](https://www.npmjs.com/package/microtap-mcp) ([source](https://github.com/swaltersjrtest/microtap-mcp)) wraps this whole catalog as [Model Context Protocol](https://modelcontextprotocol.io) tools, so any MCP-speaking agent (Claude, Cursor, etc.) can call, pay for, and read these endpoints directly — `npx -y microtap-mcp`, no separate install or API integration needed.
...
| `/api/web-search` | $0.003 | Real ranked web search: title, URL, published date, author, relevance score | Exa (requires `EXA_API_KEY`) |
| `/api/prediction-arb` | $0.005 | Finds the same real-world question priced differently on Polymarket vs. Kalshi and ranks by odds divergence | Polymarket Gamma API + Kalshi public API |
...
20 monetized endpoints total. Every route's exact input schema and a live example call/response is discoverable via its `extensions.bazaar` metadata in the 402 response — see "Discovery & trust" below.
...
app/
  ...
  api/{polymarket-markets,polymarket-market,kalshi-markets,kalshi-market,defi-tvl,defi-chains,
       crypto-price,crypto-market,crypto-trending,web-search,prediction-arb}/route.ts  # 11 data endpoints (prediction markets, DeFi, crypto, search — see "All monetized endpoints" for upstreams)
...
- **Endpoint strategy (August 2026 pivot):** the original 23 generic dev-tool utility endpoints (UUID/hash/base64/QR-code/etc.) were retired after reviewing real x402-ecosystem usage data on [x402-list.com](https://x402-list.com) — they matched zero proven-demand categories and generated no volume. BlockRun, the dominant Coinbase-facilitator x402 seller by volume, gets nearly all of its traffic from web search, prediction markets, and DeFi/crypto data. The 10 endpoints added in this round (`/api/polymarket-markets`, `/api/polymarket-market`, `/api/kalshi-markets`, `/api/kalshi-market`, `/api/defi-tvl`, `/api/defi-chains`, `/api/crypto-price`, `/api/crypto-market`, `/api/crypto-trending`, `/api/web-search`) mirror those same proven categories, all backed by free, no-API-key-required public APIs (Polymarket Gamma, Kalshi, DefiLlama, CoinGecko) except `/api/web-search`, which originally used DuckDuckGo's Instant Answer API and was later replaced — see the next note — priced in the same $0.001–$0.003 range BlockRun uses.
...
- **`/api/prediction-arb` (August 2026):** the growth-strategy doc's top differentiation recommendation — nothing else in the current catalog crosses Polymarket and Kalshi to surface where the *same* real-world question is priced differently on each platform. Matching is nontrivial: plain title text differs between platforms, Kalshi's default `/markets` listing is dominated by thousands of auto-generated zero-volume sports "combo" markets (fixed by querying `/events?with_nested_markets=true` and filtering to a `category` allowlist instead), and Kalshi events with multiple sub-markets (e.g. one per candidate) share an identical `title` — the actual distinguishing text lives only in `yes_sub_title`, which is folded into matching and also surfaced in the response as `subTitle`. Matching itself uses IDF-weighted token overlap (not plain Jaccard) so generic recurring framing words score lower than the rare words that actually distinguish one market from another; validated iteratively against live production data from both APIs. **Verification note:** this route passed `tsc`/lint/build, a local structural test (correct 402 challenge, correct price/metadata), and extensive live-data validation of the matching algorithm itself — but, unlike the routes verified in "Verified end-to-end" below, it has not yet been exercised through a full authenticated payment (no funded wallet was available in the environment it was built in). Treat it as logic-verified but not yet payment-flow-verified until a real settled call is confirmed on-chain.
- **`/api/web-search` backend swap (August 2026):** this route was the last one without a confirmed real settlement, and the root cause turned out to be a content gap, not just a reliability bug — DuckDuckGo's Instant Answer API only returns a pre-canned abstract for queries matching a known topic (mostly Wikipedia subjects) and returns essentially nothing for an ordinary search query, so it was never really serving the "web search" category buyers actually pay for. Replaced with a direct proxy to [Exa](https://exa.ai)'s `/search` endpoint — the same real-ranked-results web search BlockRun's own top-volume catalog sells — returning title/URL/published date/author/relevance score per result. Requires an `EXA_API_KEY` (free tier, no card required — see `.env.local.example`); the route fails closed with an uncharged 502 if that key is missing, same "no charge on failure" guarantee as every other route.
